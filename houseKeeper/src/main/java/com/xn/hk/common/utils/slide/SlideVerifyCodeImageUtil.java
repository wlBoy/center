 ...
 */
public class SlideVerifyCodeImageUtil {
	/**
	 * 模板图宽度
	 */
	 ...
	 * 
	 * @param originImgDirectory
	 *            原图片目录
	 * @return
	 * @throws Exception
	 */
	public static SlideVerifyCodeImage getRandomSlideVerifyCodeImage(String originImgDirectory) throws Exception {
		return getRandomSlideVerifyCodeImage(originImgDirectory, false, null);
	}
	 ...
	 * @throws Exception
	 */
	public static SlideVerifyCodeImage getRandomSlideVerifyCodeImage(String originImgDirectory, boolean isSaveImage,
		...
		BufferedImage originImage = getBufferedImage(randomImgPath);
		// 生成拼图样式，并进行图片裁剪处理
		...
		if (isSaveImage) {
			...
			String name = randomImgName.substring(0, randomImgName.indexOf('.'));
		...
		return new SlideVerifyCodeImage(getImageBase64(originImage), getImageBase64(markImage), locationX, locationY);
	}
	...
	public static void main(String[] args) throws Exception {
		// 原图片目录
		String directoryPath = "D:/picture/project";
		// 裁剪之后图片保存目录
		String savePath = "D:/picture/cut";
		System.out.println(getRandomSlideVerifyCodeImage(directoryPath, true, savePath));
	}
