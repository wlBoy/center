...
public class SnmpTask {
	...
	private static String snmpAgentAdress = SystemCfg.loadCfg().getProperty(SystemCfg.MONITOR_IP);
	...
	public void moinitor() throws IOException {
		if (!Boolean.valueOf(SystemCfg.loadCfg().getProperty(SystemCfg.ENABLE_MONITOR))) {
	...
	private List<EnumStatusItem> getMonitorItems() {
		...
		if (Boolean.valueOf(SystemCfg.loadCfg().getProperty(SystemCfg.MONITOR_CPU))) {// 开启监听CPU
			items.add(EnumStatusItem.CPU);
		}
		if (Boolean.valueOf(SystemCfg.loadCfg().getProperty(SystemCfg.MONITOR_DISK))) {// 开启监听硬盘
			items.add(EnumStatusItem.DISK);
		}
		if (Boolean.valueOf(SystemCfg.loadCfg().getProperty(SystemCfg.MONITOR_NETWORK_FLOW))) {// 开启监听网络流量
			items.add(EnumStatusItem.NETWORKFLOW);
		}
		if (Boolean.valueOf(SystemCfg.loadCfg().getProperty(SystemCfg.MONITOR_SERVICE_TIME))) {// 开启监听设备服务时间
			items.add(EnumStatusItem.SYSUPTIME);
		}
		if (Boolean.valueOf(SystemCfg.loadCfg().getProperty(SystemCfg.MONITOR_MEMORY))) {// 开启监听内存
